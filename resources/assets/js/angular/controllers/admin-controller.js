...
admin.controller('QuestionsController', [
	...
	function($scope, Notification, sendRequest, bootstrapAdminPage) {
		...
		$scope.editQuestion = () => {
			...
			sendRequest.postRequest(route_root + '/api/edit-question', $scope.q).then(rsp => {
				if (rsp.status == 200) {
		...
		$scope.createQuestion = () => {
			...
			sendRequest.postRequest(route_root + '/api/create-question', $scope.q).then(rsp => {
				if (rsp.status == 200) {
		...
		$scope.deleteQuestion = q => {
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/delete-question', q).then(rsp => {
				console.log(rsp)
				if (rsp.status == 200) {
...
admin.controller('AdminsController', [
	'$scope',
	'Notification',
	'sendRequest',
	'bootstrapAdminPage',
	function($scope, Notification, sendRequest, bootstrapAdminPage) {
		...
		$scope.editAdmin = () => {
			...
			sendRequest.postRequest(route_root + '/api/edit-admin', $scope.q).then(rsp => {
				if (rsp.status == 200) {
					Notification.warning('Edited')
					$scope.correct = null
					$scope.q = null
		...
		$scope.deleteAdmin = q => {
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/delete-admin', q).then(rsp => {
				if (rsp.status == 200) {
		...
		$scope.removeAdmin = q => {
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/remove-admin', q).then(rsp => {
				if (rsp.status == 200) {
...
admin.controller('UsersController', [
	...
	function($scope, $timeout, Notification, sendRequest, bootstrapAdminPage) {
		...
		$scope.editUser = () => {
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/edit-user', $scope.u).then(rsp => {
				if (rsp.status == 200) {
					Notification.warning('Edited')
					$scope.correct = null
					$scope.u = null
		...
		$scope.openReferralsModal = u => {
			sendRequest.postRequest(route_root + '/api/get-user-referrals', u.id).then(rsp => {
				if (rsp.status == 200) {
					$scope.u = u
		...
		$scope.deleteUser = u => {
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/delete-user', u).then(rsp => {
				if (rsp.status == 200) {
					Notification.warning('Deleted')
		...
		$scope.suspendUser = u => {
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/suspend-user', u).then(rsp => {
				console.log(rsp)
				if (rsp.status == 200) {
		...
		$scope.activateUser = u => {
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/activate-user', u).then(rsp => {
				console.log(rsp)
				if (rsp.status == 200) {
		...
		$scope.verifyUser = u => {
			...
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/verify-user', u).then(rsp => {
				if (rsp.status == 200) {
		...
		$scope.verifyAllUsers = () => {
			$scope.verifying = true
			NProgress.start()
			sendRequest.putRequest(route_root + '/api/verify-all-users').then(rsp => {
				if (rsp.status == 200) {
		...
		$scope.processCreditAddition = u => {
			$scope.loading = true
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/create-transaction', u).then(rsp => {
				if (rsp.status == 200) {
		...
		$scope.makeAdmin = u => {
			NProgress.start()
			sendRequest.postRequest(route_root + '/api/create-admin', u).then(rsp => {
				if (rsp.status == 200) {
...
admin.controller('GamesController', [
	'$scope',
	'Notification',
	'sendRequest',
	'bootstrapAdminPage',
...
admin.controller('TransactionsController', [
	'$scope',
	'Notification',
	'sendRequest',
	'bootstrapAdminPage',
...
admin.controller('MessagesController', [
	'$scope',
	'Notification',
	'sendRequest',
	'bootstrapAdminPage',
...
admin.controller('EarningsController', [
	'$scope',
	'Notification',
	'sendRequest',
	'bootstrapAdminPage',
