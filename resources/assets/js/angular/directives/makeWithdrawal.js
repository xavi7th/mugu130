...
angular.module('makeWithdrawal', []).directive('makeWithdrawal', [
	'$timeout',
	'$location',
	'Notification',
	'sendRequest',
	function($timeout, $location, Notification, sendRequest) {
		return {
			restrict: 'E',
			scope: {
				// dest : '=',
			...
			controller: [
				...
				$scope => {
					...
					$scope.requestWithdrawal = () => {
						sendRequest
							.postRequest('/user/request-withdrawal', {
									...
									if (rsp.data.status) {
										if ($scope.requested_amount < 1000) {
											Notification.primary(
												'Amount requested will be sent as airtime to ' +
													$scope.$parent.userdetails.phone1
											)
										} else {
											...
											Notification.primary({
												message:
													'Amount requested will be sent to account number (' +
													$scope.$parent.userdetails.acct_no +
													' - ' +
													$scope.$parent.userdetails.bank +
													') within 72 hours',
												positionX: 'center'
											})
										}

										$scope.$parent.userdetails.available_units =
											$scope.$parent.userdetails.available_units -
					...
					$timeout(function() {
						...
						$scope.withdrawal_max =
							$scope.$parent.userdetails.available_units < 50000
								? $scope.$parent.userdetails.available_units
								: 50000
					}, 1000)
