...
class Transaction extends Model
{
	...
	/**
	 * Retrieves all approved cash out requests from the transaction database.
	 *
	 * Gets a paginate object from the transaction table based on transaction type being withdrawala and the status being pending
	 *
	 * @return return PaginatedResourceResponse
	...
	public static function approvedCashouts()
	{
		return self::with(['user'])->where('trans_type', 'withdrawal')->where('status', 'completed')->paginate(env('ROWS_PER_PAGE'));
	}
